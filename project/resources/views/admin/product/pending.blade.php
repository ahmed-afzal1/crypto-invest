...
<div class="card">
	...
	<h5 class=" mb-0 text-gray-800 pl-3">{{ __('Pending Products') }}</h5>
	<ol class="breadcrumb">
		<li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }}</a></li>
		<li class="breadcrumb-item"><a href="javascript:;">{{ __('Products') }}</a></li>
		<li class="breadcrumb-item"><a href="{{ route('admin.product.pending') }}">{{ __('Pending Product') }}</a></li>
	</ol>
	</div>
</div>
...
{{-- DATA TABLE --}}
    ...
    <script type="text/javascript">
		...
		var table = $('#geniustable').DataTable({
               ...
               serverSide: true,
               searching: false,
               ajax: '{{ route('admin.product.datatables','pending') }}',
               columns: [
